...
//Alt + X 重新开始游戏（当角色死亡后特别有用）
      ...
      // word('V'): begin
      //  if not PlayScene.EdChat.Visible then
      //    FrmDlg.OpenFriendDlg();    //打开朋友对话框
      // end;

      word('N'): begin
        if not PlayScene.EdChat.Visible then
          FrmDlg.OpenAdjustAbility;    //调整属性对话框
      end;
   end;
...
begin
   ...
   g_boSendLogin:=True;
   // debug send login 
   // FrmDlg.DMessageDlg ('SendLogin', [mbOk]);
end;
...
//	  FrmDlg.DMessageDlg (IntToStr(msg.Ident), [mbOk]);
      case msg.Ident of
         SM_NEWID_SUCCESS:
            begin
         ...
         SM_NEWCHR_FAIL: begin
           case msg.Recog of
             ...
             2: FrmDlg.DMessageDlg ('[错误] 创建的角色名称已被使用!', [mbOk]);
...
begin
	// debug
//   FrmDlg.DMessageDlg ('ClientGetPasswdSuccess', [mbOk]);
   str := DecodeString (body);
...
var
  ...
  nCount:Integer;
begin
//FrmDlg.DMessageDlg ('SM_PASSOK_SELECTSERVER', [mbOk]);
...
//  end;
	...
	//FrmDlg.DMessageDlg (sServerName, [mbOk]);
	//FrmDlg.DMessageDlg (sServerStatus, [mbOk]);
