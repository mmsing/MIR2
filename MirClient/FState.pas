...
type
  ...
  TFrmDlg = class(TForm)
    ...
    DOption: TDButton;
    DGold: TDButton;
    DPrevState: TDButton;    //状态窗口,左边装备、状态信息、人物属性PageUp
    DNextState: TDButton;    //状态窗口,左边装备、状态信息、人物属性PageDown
    ...
    DBotExit: TDButton;
    DBotGuild: TDButton;
    DStPageUp: TDButton;       //状态窗口,右边魔法PageUp
    DStPageDown: TDButton;     //状态窗口,右边魔法PageDown
...
begin
   ...
   //对话框的位置是在主窗口内的相对位置，即相对于主窗口顶点(0,0)坐标的位置
   //DLogIn.SetImgIndex (g_WMainImages, 60);  //登录对话框
   //DLogIn.Left := 250;
   //DLogIn.Top := 170;

   //按钮的从标是在对话框内的相对位置。即相对于对话框顶点(0,0)坐标的位置
   //这里设置图片是按钮被点击时显示的图片。
   //标加1是使按钮被点击向左、下各移一个点，使其有动的感觉，点击完成后这个图标会自动消失。
   DLoginNew.SetImgIndex (g_WMainImages, 61);  //[新用户]按钮
   //DLoginNew.Visible := TRUE;
   DLoginNew.Left := 24+1;
   DLoginNew.Top  := 206+1;
   DLoginOk.SetImgIndex (g_WMainImages, 62);  //[提交]按钮
   DLoginOk.Left := 168+1;
   DLoginOk.Top  := 162+1;
   DLoginChgPw.SetImgIndex (g_WMainImages, 53); //[修改密码]按钮
   DLoginChgPw.Left := 129+1;
   DLoginChgPw.Top  := 206+1;
   DLoginClose.SetImgIndex (g_WMainImages, 64); //[X]关闭按钮
   DLoginClose.Left := 252;        //关闭按钮坐标不用加1，因为在图片中作了内凹效果
   DLoginClose.Top  := 28;
   ...
   //英(中)文对话框：选择服务器
      ...
      DSSrvClose.SetImgIndex (g_WMainImages, 64);   //[X]关闭按钮
      DSSrvClose.Left := 245;
      DSSrvClose.Top := 31;
{
   ...
   DNewAccountOk.SetImgIndex (g_WMainImages, 62); //[提交]按钮
   DNewAccountOk.Left := 157+1;
   DNewAccountOk.Top  := 415+1;
   DNewAccountCancel.SetImgIndex (g_WMainImages, 52); //[取消]按钮
   DNewAccountCancel.Left := 445+1;
   DNewAccountCancel.Top  := 418+1;
   DNewAccountClose.SetImgIndex (g_WMainImages, 64);  //[X]关闭按钮
   ...
   DChgpwOk.SetImgIndex (g_WMainImages, 81);   //[同意]按钮
   DChgPwOk.Left := 180+1;
   DChgPwOk.Top  := 252+1;
   DChgpwCancel.SetImgIndex (g_WMainImages, 52); //[取消]按钮
   DChgPwCancel.Left := 275+1;
   DChgPwCancel.Top  := 251+1;
   ...
   DscSelect1.SetImgIndex (g_WMainImages, 66);    //左边[选择] 钮按
   DscSelect2.SetImgIndex (g_WMainImages, 67);    //右边[选择] 钮按
   DscStart.SetImgIndex (g_WMainImages, 68);      //[开始] 钮按
   DscNewChr.SetImgIndex (g_WMainImages, 69);     //[创建人物] 钮按
   DscEraseChr.SetImgIndex (g_WMainImages, 70);   //[删除人物] 钮按
   //DscCredits.SetImgIndex (g_WMainImages, 71);    //[制作群] 钮按
   DscExit.SetImgIndex (g_WMainImages, 72);       //[退出] 钮按

      DscSelect1.Left := (SCREENWIDTH  - 800) div 2 + 133;  // 134{134};  //左[选择]
      DscSelect1.Top  := (SCREENHEIGHT - 600) div 2 + 453;  // 424{454};

      DscSelect2.Left := (SCREENWIDTH  - 800) div 2 + 685;  // 602{685};  //右[选择]
      DscSelect2.Top  := (SCREENHEIGHT - 600) div 2 + 454;  // 424{454};

      DscStart.Left   := (SCREENWIDTH  - 800) div 2 + 385;  // 374{385};  //[开始]
      DscStart.Top    := (SCREENHEIGHT - 600) div 2 + 456;  // 427{456};
      
      DscNewChr.Left  := (SCREENWIDTH  - 800) div 2 + 348;  // 349{348};  //[创建人物]
      DscNewChr.Top   := (SCREENHEIGHT - 600) div 2 + 486;  // 467{486};
      
      DscEraseChr.Left := (SCREENWIDTH  - 800) div 2 + 347; // 349{347};  //[删除人物]
      DscEraseChr.Top  := (SCREENHEIGHT - 600) div 2 + 506; // 505{506};

     // DscCredits.Left := (SCREENWIDTH - 800) div 2 + 362;  // 362{362}; //[制作群]
     // DscCredits.Top := (SCREENHEIGHT - 600) div 2 + 527;  // 529{527};
     
      DscExit.Left := (SCREENWIDTH - 800) div 2 + 379;       // 349{379}; //[退出]
      DscExit.Top := (SCREENHEIGHT - 600) div 2 + 547;       // 543{547};
   ...
   DccWarrior.SetImgIndex (g_WMainImages, 74);        //战士
   DccWizzard.SetImgIndex (g_WMainImages, 75);        //法师
   DccMonk.SetImgIndex (g_WMainImages, 76);           //道士（术士）
   //DccReserved.SetImgIndex (g_WMainImages.Images[76], TRUE);
   
   DccMale.SetImgIndex (g_WMainImages, 77);        //男
   DccFemale.SetImgIndex (g_WMainImages, 78);      //女

   DccLeftHair.SetImgIndex (g_WMainImages, 79);     //选择前一个发型
   DccRightHair.SetImgIndex (g_WMainImages, 80);    //选择后一个头型
  
   DccOk.SetImgIndex (g_WMainImages, 62);         //确定
   DccClose.SetImgIndex (g_WMainImages, 64);      //关闭
      DccWarrior.Left := 48; //36;                //战
      DccWarrior.Top  := 157; //139;
      DccWizzard.Left := 93;                      //法
      DccWizzard.Top  := 157;
      DccMonk.Left    := 138;                     //道
      DccMonk.Top     := 157;
   ...
   //人物状态窗口
   d := g_WMainImages.Images[370];  //370基本状态窗口
   if d <> nil then begin
      ...
      //外窗口左边宽度：38，外窗口上边宽度：52
      DSWNecklace.Left := 38 + 131; //169; //38 + 144;
      DSWNecklace.Top  := 52 + 36; //88; //52 + 13;    //项链
      DSWNecklace.Width := 34;
      DSWNecklace.Height := 30;

      //头盔可选择区域
      DSWHelmet.Left := 38 + 77;   //头盔
      DSWHelmet.Top  := 52 + 33; //21;
      DSWHelmet.Width := 18;
      DSWHelmet.Height := 18;

      DSWLight.Left := 38 + 131; //169; //38 + 144;     //火把
      DSWLight.Top  := 52 + 75;  //127; //52 + 53;
      DSWLight.Width := 34;
      DSWLight.Height := 30;

      DSWArmRingR.Left := 38 + 5;   //43;  //25+ 0;     //左手镯
      DSWArmRingR.Top  := 52 + 125; //177; //52 + 89;
      DSWArmRingR.Width := 34;
      DSWArmRingR.Height := 30;

      DSWArmRingL.Left := 38 + 131; //169; //38 + 144;  //右手镯 (术士可放：道符、毒)
      DSWArmRingL.Top  := 52 + 125; //177; //52 + 89;
      DSWArmRingL.Width := 34;
      DSWArmRingL.Height := 30;

      DSWRingR.Left := 38 + 5;    //43; //25 + 0;       //左戒指
      DSWRingR.Top  := 52 + 165;  //217; //52 + 128;
      DSWRingR.Width := 34;
      DSWRingR.Height := 30;

      DSWRingL.Left := 38 + 131;  //169; //38 + 144;    //右戒指
      DSWRingL.Top  := 52 + 165;  //217; //52 + 128;
      DSWRingL.Width := 34;
      DSWRingL.Height := 30;

      DSWWeapon.Left := 38 + 9;       //武器
      DSWWeapon.Top  := 52 + 18;
      DSWWeapon.Width := 47;
      DSWWeapon.Height := 87;

      //衣服可选择区域
      DSWDress.Left := 38 + 58; // + 58;       //衣服
      DSWDress.Top  := 52 + 70; //+ 70;
      DSWDress.Width := 53;
      DSWDress.Height := 112;
...
//      DSWBoots.Left := 128;       //鞋子
//      DSWBoots.Top  := 232;
//      DSWBoots.Width := 34;
//      DSWBoots.Height := 30;

//      DSWCharm.Left := 182;         //宝石
//      DSWCharm.Top  := 232;
//      DSWCharm.Width := 34;
//      DSWCharm.Height := 30;
...
// TFrmDlg.DSWWeaponClick()       点击选取装备
      ...
      //魔法技能翻页按钮(位于右边)
      DStPageUp.SetImgIndex (g_WMainImages, 398);   //   ^ (箭头向上)
      DStPageDown.SetImgIndex (g_WMainImages, 396); //   V (箭头向下)
      DStPageUp.Left := 213;
      DStPageUp.Top  := 113;
      ...
      //人物装备、壮态翻页按钮(位于左边)
      DPrevState.SetImgIndex (g_WMainImages, 373);    //^ (箭头向上)
      DNextState.SetImgIndex (g_WMainImages, 372);    //V (箭头向下)
      DPrevState.Left := 7;
      DPrevState.Top := 128;
      DNextState.Left :=7;
      DNextState.Top := 187;
   ...
   DCloseState.SetImgIndex (g_WMainImages, 371);     //[X]关闭
   DCloseState.Left := 8;
   DCloseState.Top  := 39;
   ...
   if d <> nil then begin
      ...
      DUserState1.Left := SCREENWIDTH - d.Width;
      DUserState1.Top := 52;
   end;

      DNecklaceUS1.Left := 38 + 130;   //项链
      DNecklaceUS1.Top  := 52 + 35;
      DNecklaceUS1.Width := 34;
      DNecklaceUS1.Height := 30;

      DHelmetUS1.Left := 38 + 77;     //头盔
      DHelmetUS1.Top  := 52 + 33;
      DHelmetUS1.Width := 18;
      DHelmetUS1.Height := 18;

      DLightUS1.Left := 38 + 131;    //火把
      DLightUS1.Top  := 52 + 75;
      DLightUS1.Width := 34;
      DLightUS1.Height := 30;

      DArmRingRUS1.Left := 38 + 5;     //左手镯
      DArmRingRUS1.Top  := 52 + 125;
      DArmRingRUS1.Width := 34;
      DArmRingRUS1.Height := 30;

      DArmRingLUS1.Left := 38 + 131;   //右手镯
      DArmRingLUS1.Top  := 52 + 125;
      DArmRingLUS1.Width := 34;
      DArmRingLUS1.Height := 30;
      
      DRingRUS1.Left := 38 + 5;     //左戒指
      DRingRUS1.Top  := 52 + 165;
      DRingRUS1.Width := 34;
      DRingRUS1.Height := 30;

      DRingLUS1.Left := 38 + 131;   //右戒指
      DRingLUS1.Top  := 52 + 165;
      DRingLUS1.Width := 34;
      DRingLUS1.Height := 30;

      DWeaponUS1.Left := 38 + 9;    //武器
      DWeaponUS1.Top  := 52 + 18;
      DWeaponUS1.Width := 47;
      DWeaponUS1.Height := 87;

      DDressUS1.Left := 38 + 58;     //衣服
      DDressUS1.Top  := 52 + 70;
      DDressUS1.Width := 53;
...
//      DBootsUS1.Left := 126;      //鞋子
//      DBootsUS1.Top  := 254;
//      DBootsUS1.Width := 34;
//      DBootsUS1.Height := 30;

//      DCharmUS1.Left := 168;         //宝石
//      DCharmUS1.Top  := 254;
//      DCharmUS1.Width := 34;
//      DCharmUS1.Height := 30;

   DCloseUS1.SetImgIndex (g_WMainImages, 371);      //[X]关闭
   DCloseUS1.Left := 8;
   DCloseUS1.Top := 39;
  ...
  {-------------------------------------------------------------}
   ...
   DItemGrid.Left := 20;  //网格
   DItemGrid.Top  := 13;
   DItemGrid.Width := 286;
   DItemGrid.Height := 163;
   ...
   DClosebag.SetImgIndex (g_WMainImages, 371); //[X] 包裹关闭
   DCloseBag.Left := 309; //314;
   DCloseBag.Top := 202;   //20;
   //DCloseBag.Width := 14;
   //DCloseBag.Height := 20;
...
{$IFEND}
   ...
   DMerchantDlgClose.SetImgIndex (g_WMainImages, 371);  //[X]关闭
   {-----------------------------------------------------------}
   //配置窗口
   ...
   DMenuPrev.Left := 43;
   DMenuPrev.Top := 175;
   DMenuPrev.SetImgIndex (g_WMainImages, 388);   //向前
  
   DMenuNext.Left := 90;
   DMenuNext.Top := 175;
   DMenuNext.SetImgIndex (g_WMainImages, 387);  //向后
  
   DMenuBuy.Left := 215;
   DMenuBuy.Top := 171;
   DMenuBuy.SetImgIndex (g_WMainImages, 386);   //[OK]购买
   
   DMenuClose.Left := 291;
   DMenuClose.Top := 0;
   DMenuClose.SetImgIndex (g_WMainImages, 371);    //关闭
   ...
   DSellDlgClose.Top := 0;
   DSellDlgClose.SetImgIndex (g_WMainImages, 371);  //[X] 关闭
   ...
   //设置魔法快捷对话框
   { }
   d := g_WMainImages.Images[229];    //快捷键对话框
   if d <> nil then begin
   ...
   DKsIcon.Left := 51;  //选择的魔法图标
   DKsIcon.Top := 28;
   //Key F1~F8
   DKsF1.SetImgIndex (g_WMainImages, 233);   //F1
   DKsF1.Left := 34;
   DKsF1.Top  := 83;
   DKsF2.SetImgIndex (g_WMainImages, 235);   //F2
   DKsF2.Left := 66;
   DKsF2.Top  := 83;
   DKsF3.SetImgIndex (g_WMainImages, 237);   //F3
   DKsF3.Left := 98;
   DKsF3.Top  := 83;
   DKsF4.SetImgIndex (g_WMainImages, 239);   //F4
   DKsF4.Left := 130;
   DKsF4.Top  := 83;
   
   DKsF5.SetImgIndex (g_WMainImages, 241);   //F5
   DKsF5.Left := 171;
   DKsF5.Top  := 83;
   DKsF6.SetImgIndex (g_WMainImages, 243);   //F6
   DKsF6.Left := 203;
   DKsF6.Top  := 83;
   DKsF7.SetImgIndex (g_WMainImages, 245);   //F7
   DKsF7.Left := 235;
   DKsF7.Top  := 83;
   DKsF8.SetImgIndex (g_WMainImages, 247);   //F8
   DKsF8.Left := 267;
   DKsF8.Top := 83;
   
   DKsNone.SetImgIndex (g_WMainImages, 230);   //NONE
   DKsNone.Left := 299;
   DKsNone.Top  := 83;
   DKsOk.SetImgIndex (g_WMainImages, 362);     //OK
   DKsOk.Left := 220;
   DKsOk.Top  := 129;  

  {
   ...
   DGrpDlgClose.SetImgIndex (g_WMainImages, 371); //[X]关闭
   DGrpDlgClose.Left := 260;
   DGrpDlgClose.Top := 0;
   DGrpAllowGroup.SetImgIndex (g_WMainImages, 122);  //允许组队
   DGrpAllowGroup.Left := 20;
   DGrpAllowGroup.Top := 18;
   DGrpCreate.SetImgIndex (g_WMainImages, 123);      //创建组
   DGrpCreate.Left := 21;
   DGrpCreate.Top := 202;
   DGrpAddMem.SetImgIndex (g_WMainImages, 124);      //添加成员
   DGrpAddMem.Left := 96;
   DGrpAddMem.Top := 202;
   DGrpDelMem.SetImgIndex (g_WMainImages, 125);     //删除成员
   DGrpDelMem.Left := 171;
   DGrpDelMem.Top := 202;
   ...
   DDealClose.SetImgIndex (g_WMainImages, 64);  //[X] 关闭
   DDealClose.Left := 220;
   DDealClose.Top := 42;
   ...
   if d <> nil then begin
      DGuildDlg.Left := 0;
      DGuildDlg.Top := 0;
      DGuildDlg.SetImgIndex (g_WMainImages, 180);   //行会框
   end;
   DGDClose.Left := 584;
   DGDClose.Top  := 6;
   DGDClose.SetImgIndex (g_WMainImages, 371);      //[x] 闭关
   DGDHome.Left := 13; //27;
   DGDHome.Top  := 411; //387;
   DGDHome.SetImgIndex (g_WMainImages, 198);       //主页
   DGDList.Left := 13; //27;
   DGDList.Top  := 429; //435; //407;
   DGDList.SetImgIndex (g_WMainImages, 200);      //成员列表
   DGDChat.Left := 94; //112;
   DGDChat.Top  := 429; //410; //407;
   DGDChat.SetImgIndex (g_WMainImages, 190);      //聊天记录
   
   DGDAddMem.Left := 243; //235;
   DGDAddMem.Top  := 411; // 410; //387;
   DGDAddMem.SetImgIndex (g_WMainImages, 182);    //添加成员
   DGDDelMem.Left := 243; //235;
   DGDDelMem.Top  := 429; //407;
   DGDDelMem.SetImgIndex (g_WMainImages, 192);    //删除成员
   DGDEditNotice.Left := 325; //320;
   DGDEditNotice.Top  := 411; //387;
   DGDEditNotice.SetImgIndex (g_WMainImages, 196);  //编辑公告
   DGDEditGrade.Left := 325; //320;
   DGDEditGrade.Top  := 429; //407;
   DGDEditGrade.SetImgIndex (g_WMainImages, 194);   //编辑封号
   DGDAlly.Left := 407; //404;
   DGDAlly.Top  := 411; //387;
   DGDAlly.SetImgIndex (g_WMainImages, 184);       //联盟
   DGDBreakAlly.Left := 407; // 404;
   DGDBreakAlly.Top  := 429; //407;
   DGDBreakAlly.SetImgIndex (g_WMainImages, 186);   //取消联盟
   
   DGDWar.Left := 529; //488;
   DGDWar.Top  := 411; //387;
   DGDWar.SetImgIndex (g_WMainImages, 202);         //战争
   DGDCancelWar.Left := 529; //488;
   DGDCancelWar.Top  := 429; //407;
   DGDCancelWar.SetImgIndex (g_WMainImages, 188);  //取消战争

   DGDUp.Left := 595; //567;
   DGDUp.Top  := 239;  //110;
   DGDUp.SetImgIndex (g_WMainImages, 373);        //向上箭头
   DGDDown.Left := 595; //587; //567;
   DGDDown.Top  := 291; //283; //364;
   DGDDown.SetImgIndex (g_WMainImages, 372);     //向下箭头

    //行会通告编辑框
   DGuildEditNotice.SetImgIndex (g_WMainImages, 204);
   DGEOk.SetImgIndex (g_WMainImages, 362);         //确定
   DGEOk.Left := 514;
   DGEOk.Top := 287;
   DGEClose.SetImgIndex (g_WMainImages, 64);       //[x] 闭关
   DGEClose.Left := 584;
   DGEClose.Top := 6;
   ...
   DAdjustAbilOk.SetImgIndex (g_WMainImages, 362);
   DAdjustAbilOk.Left := 218; //255;
   DAdjustAbilOk.Top := 296; //301;

   DPlusDC.SetImgIndex (g_WMainImages, 227); //224
   DPlusDC.Left := 217;
   DPlusDC.Top := 101;
   ...
   DPlusHit.Left := 217;
   DPlusHit.Top := 240;
   DPlusSpeed.SetImgIndex (g_WMainImages, 227);   //224
   DPlusSpeed.Left := 217;
   DPlusSpeed.Top := 261;

   DMinusDC.SetImgIndex (g_WMainImages, 228);  //225
   DMinusDC.Left := 227;
   DMinusDC.Top := 101;
   ...
   DMinusHit.Left := 227;
   DMinusHit.Top := 240;
   DMinusSpeed.SetImgIndex (g_WMainImages, 228);  //225
   ...
   with dsurface.Canvas do begin
      ...
      for i:=0 to NAHelps.Count-1 do begin
         ...
         //TextOut ((SCREENWIDTH div 2 - 289) + 386 + 10, (SCREENHEIGHT div 2 - 300) + 119 + 5 + i*14, NAHelps[i]);           
          TextOut ((SCREENWIDTH div 2 - 320) + 386 + 10, (SCREENHEIGHT div 2 - 237) + 119 + 5 + i*14, NAHelps[i]);    //modified  by davy 2019-11-25
      end;         //上问题提示
      //BoldTextOut (dsurface, 79+283, 64 + 57, clWhite, clBlack, NewAccountTitle);  //modified  by davy 2019-11-25
      Release;
   end;
end;
   ...
   with DStateWin do begin
      ...
      case StatePage of
         0: begin //装备
            ...
            if d <> nil then
               dsurface.Draw (SurfaceX(bbx+ax), SurfaceY(bby+ay), d.ClientRect, d, FALSE);   //人像背景

            bbx := bbx + 29+2; //29;
            bby := bby + 74 + 22; //74; //人物头发
            ...
            if g_UseItems[U_DRESS].S.Name <> '' then begin
               idx := g_UseItems[U_DRESS].S.Looks; //渴 if Myself.m_btSex = 1 then idx := 80; 
               if idx >= 0 then begin
         ...
         1: begin //Page 1 攻击属性地方  (人物状态 基础信息)
            l := Left + 115;
            m := Top + 98 ;
            with dsurface.Canvas do begin
               SetBkMode (Handle, TRANSPARENT);
               Font.Color := clWhite;
               TextOut (SurfaceX(l+0), SurfaceY(m+0), IntToStr(LoWord(g_MySelf.m_Abil.AC)) + '-' + IntToStr(HiWord(g_MySelf.m_Abil.AC)));      //AC
               TextOut (SurfaceX(l+0), SurfaceY(m+20), IntToStr(LoWord(g_MySelf.m_Abil.MAC)) + '-' + IntToStr(HiWord(g_MySelf.m_Abil.MAC)));   //MAC
               TextOut (SurfaceX(l+0), SurfaceY(m+40), IntToStr(LoWord(g_MySelf.m_Abil.DC)) + '-' + IntToStr(HiWord(g_MySelf.m_Abil.DC)));     //DC
               TextOut (SurfaceX(l+0), SurfaceY(m+60), IntToStr(LoWord(g_MySelf.m_Abil.MC)) + '-' + IntToStr(HiWord(g_MySelf.m_Abil.MC)));     //MC
               TextOut (SurfaceX(l+0), SurfaceY(m+80), IntToStr(LoWord(g_MySelf.m_Abil.SC)) + '-' + IntToStr(HiWord(g_MySelf.m_Abil.SC)));     //SC
               TextOut (SurfaceX(l+0), SurfaceY(m+100), IntToStr(g_MySelf.m_Abil.HP) + '/' + IntToStr(g_MySelf.m_Abil.MaxHP));                 //HP
               TextOut (SurfaceX(l+0), SurfaceY(m+120), IntToStr(g_MySelf.m_Abil.MP) + '/' + IntToStr(g_MySelf.m_Abil.MaxMP));                 //MP
               Release;
            end;
         end;
         2: begin //Page 2 经验 筐 移动
            bbx := Left + 0;
            bby := Top + 0;

            d := g_WMainImages.Images[382];
            if d <> nil then
                dsurface.Draw (SurfaceX(bbx+ax), SurfaceY(bby+ay), d.ClientRect, d, FALSE);
         ...
         3: begin //Page 3 魔法相关地方
            bbx := Left + 0;
            bby := Top + 0;

            d := g_WMainImages.Images[383];
            if d <> nil then
               dsurface.Draw (SurfaceX(bbx+ax), SurfaceY(bby+ay), d.ClientRect, d, FALSE);  //魔法框背景
            ...
            for i:=magtop to magline-1 do begin
               ...
               if keyimg > 0 then begin
                  ...
                  if d <> nil then
                     dsurface.Draw (bbx + 183, bby+61+m*37, d.ClientRect, d, TRUE);     //功能键图标 ：F1~ F8
               end;
            ...
            with dsurface.Canvas do begin
               ...
               for i:=magtop to magline-1 do begin
                  ...
                  if pm.Def.MaxTrain[trainlv] > 0 then begin
                     if trainlv < 3 then
                        TextOut (bbx + 87 + 46, bby + 62 + 15 + m*37, IntToStr(pm.CurTrain) + '/' + IntToStr(pm.Def.MaxTrain[trainlv]))   //魔法技能经验值
                     else TextOut (bbx + 85 + 46, bby + 62 + 15 + m*37, '-');
                  end;
               end;
               Release;
            end;
...
begin
   ...
   case StatePage of
      3: begin //魔法技能页
         ...
         DStPageUp.Visible := TRUE;     //技能前翻页
         DStPageDown.Visible := TRUE;   //技能后翻页
         MagicPage := 0;
      end;
    else begin
...
//   if Sender = DSWCharm then sel := U_CHARM;
   ...
   if sel >= 0 then begin
      ...
      if iname <> '' then begin
         ...
         //----------------------------------------------------------------------
         //此代码功能是，当光标移到人物装备上时，显示装备提示信息。
         //由于在函数TFrmDlg.DStateWinDirectPaint()中打开了在状态窗下面显示信息，
         //所以这里将光标提示信息关闭。modified by lzx 2019-12-18
        {
         // with Butt as TDButton do
         //   DScreen.ShowHint(nHintX -30, nHintY + 20,
         ...
         1..3:
            begin
              ...
              0: begin
                ...
                else if (g_MouseItem.S.AC > 0) and (g_MouseItem.S.MAC > 0) then
...
3  为道术限制
         ...
         54,64:   //腰带
            begin
               ...
               if (g_MouseItem.S.Source <= -51) and (g_MouseItem.S.Source >= -100) then
                  line2 := line2 + '诅咒-' + IntToStr(-g_MouseItem.S.Source - 50);
...
begin
   ...
   with DItemBag do begin
      ...
      with dsurface.Canvas do begin
         SetBkMode (Handle, TRANSPARENT);
         Font.Color := clWhite;
         TextOut (SurfaceX(Left+65), SurfaceY(Top+185), GetGoldStr(g_MySelf.m_nGold)); //钱数字位置（钱包金币数值）
         if d0 <> '' then begin
            n := TextWidth (d0);
            Font.Color := clYellow;
      ...
      if not BoStorageMenu then begin
         ...
         for i:=MenuTop to MenuTop+menuline-1 do begin
            ...
            if i = MenuIndex then begin
               Font.Color := clRed;
               TextOut (SX(15),  SY(32 + m*lh), char(7));   //选择标记
            end else Font.Color := clWhite;
            pg := PTClientGoods (MenuList[i]);
            TextOut (SX(27),  SY(32 + m*lh), pg.Name);     //物品

            if pg.SubMenu >= 1 then
               TextOut (SX(137), SY(50 + m*lh), #31);
            TextOut (SX(165), SY(32 + m*lh), IntToStr(pg.Price) + ' ' + g_sGoldName{金币'}); //价钱
            str := '';
            if pg.Grade = -1 then str := '-'
            else TextOut (SX(255), SY(32 + m*lh), IntToStr(pg.Grade));    //持久力
         ...
         for i:=MenuTop to MenuTop+menuline-1 do begin
            ...
            if i = MenuIndex then begin
               Font.Color := clRed;
               TextOut (SX(15),  SY(32 + m*lh), char(7));//方 选择标记
            end else Font.Color := clWhite;
            pg := PTClientGoods (MenuList[i]);
            TextOut (SX(27),  SY(30 + m*lh), pg.Name);  //物品
            if pg.SubMenu >= 1 then
               TextOut (SX(137), SY(32 + m*lh), #31);
            TextOut (SX(170), SY(32 + m*lh), IntToStr(pg.Stock) + '/' + IntToStr(pg.Grade));   //持久
         end;
      end;
...
begin
   with DSellDlg do begin
      ...
      with dsurface.Canvas do begin
         ...
         TextOut (SurfaceX(Left+6), SurfaceY(Top+6), actionname + g_sSellPriceStr);
         Release;
      end;
   end;
end;
...
begin
     ...
     //---------------------------------------------------------
     //此代码打开后，则光标提示功能需要关闭。参见函数 TFrmDlg.DWeaponUS1MouseMove()
     //恢复原来版本的显示方式。modified by lzx 2019-12-18
...
//   if Sender = DCharmUS1 then sel := U_CHARM;
   ...
   if sel >= 0 then begin
      ...
      if iname <> '' then begin
         ...
         //----------------------------------------------------------------------
         //此代码功能是，当光标移到他人物装备上时，显示装备提示信息。
         //由于在函数TFrmDlg.DUserState1DirectPaint中打开了在状态窗下面显示信息，
...
begin
   ...
   with dsurface.Canvas do begin
      ...
      m := DAdjustAbility.SurfaceY(DAdjustAbility.Top)  + 104;  //101

     {
      ...
      Font.Color := clWhite;
      l := DAdjustAbility.SurfaceX(DAdjustAbility.Left) + 170;  //195; //66;
      m := DAdjustAbility.SurfaceY(DAdjustAbility.Top)  + 104;  //101
...
//  if Sender = DBotFriend then sMsg:= '好友(V)';    // lzx 2019-11-10
  ...
  nHintY:=Butt.SurfaceY(Butt.Top) + DBottom.SurfaceY(DBottom.Top) + nLocalY +20;

  //如何按下Alt键则显示提示, 否则清除提示
  if (GetKeyState(VK_MENU)<0) then  begin //modified by lzx 2019-12-16
